...
func (s *Service) ListCheckinQueue(ctx context.Context, tpsID int64, status string, page, limit int) (*CheckinQueueResponse, error) {
	if page < 1 {
		page = 1
	}
	if limit < 1 {
...
func (s *Service) ApproveCheckin(ctx context.Context, tpsID, checkinID, approverID int64) (*ApproveCheckinResponse, error) {
	// Verify access
	hasAccess, _ := s.repo.IsPanitiaAssigned(ctx, tpsID, approverID)
	if !hasAccess {
...
func (s *Service) GetTPSSummary(ctx context.Context, tpsID, userID int64) (*TPSSummaryResponse, error) {
